  ...
  return (
    <nav className="flexBetween px-6 lg:px-20 xl:px-20 relative z-30 py-5 h-[90px] shadow">
      ...
      <ul className="hidden h-full gap-12 lg:flex">
        {NAV_LINKS.map((link) => (
          <Link href={link.href} key={link.key} className="text-lg font-bold text-gray-500 flexCenter cursor-pointer pb-1.5 transition-all hover:font-bold hover:text-orange-400 hover:text-xl">
            {link.label}
          </Link>
        ))}
      </ul>
        ...
        <div className="lg:flexCenter hidden">
          ...
          {isOpen && (
            <div className='absolute top-20 flex flex-col py-2 border rounded bg-white animate-scale-up-ver-top'>
              {DROPDOWN.map((link) => (
                <Link href={link.href} key={link.href} className="py-2 px-3 font-semibold hover:bg-blue-600 hover:text-white">
                  <Image
                    src={link.image}
                    alt="icon"
                    width={23}
